...
print(f"Starting Homophily Sweep for Seed: {target_seed}...")

for h_val in homophily_levels:
    print(f"  Running Condition: homophily = {h_val:.1f}")
    run_data = run_simulation(seed=target_seed, homophily=h_val) 
...
summary_path = os.path.join(script_dir, f"exp_homophily_micro_seed_{target_seed}.csv")
...
ax1.legend()
fig.suptitle(f"Impact of Homophily on System Consensus (Seed: {target_seed})", fontweight='bold', fontsize=16)
plt.tight_layout()
plt.show()