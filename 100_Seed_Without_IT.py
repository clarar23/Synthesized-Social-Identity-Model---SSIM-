...
# =====================================================================
print(f"Starting IT Ablation sweep for {num_seeds} random seeds...")
print("Condition: Identity Theory DISABLED (beta=0)")

for i in range(num_seeds):
    random_seed = np.random.randint(0, 1000000)
    # --- THE ABLATION TOGGLE ---
    run_data = run_simulation(seed=random_seed, beta =0) 
...
save_path = os.path.join(script_dir, "ablation_no_IT_results.csv")
...
stats_path = os.path.join(script_dir, "ablation_no_IT_summary_stats.csv")
...
fig.suptitle(f"Ablation Study: Identity Theory DISABLED (100 Seeds)", fontsize=16, fontweight='bold')
